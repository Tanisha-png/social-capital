...
import * as authService from "../../services/authService";
import Avatar from "../../components/Avatar/Avatar";
import { getAvatarPreview, cleanupPreview } from "../../components/Avatar/Avatar";
import "./SignUpPage.css";
...
export default function SignUpPage() {
  ...
  const avatarUrl = avatarPreview || getAvatarPreview(null, formData.name || "user");
  ...
  function handleChange(e) {
    ...
    if (name === "avatarFile" && files?.[0]) {
      const f = files[0];
      setAvatarFile(f);
      setAvatarPreview(getAvatarPreview(f, formData.name || "user"));
    } else {
      setFormData((p) => ({ ...p, [name]: value }));
    }
  }
