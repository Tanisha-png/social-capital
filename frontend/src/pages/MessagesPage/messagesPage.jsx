...
export default function MessagesPage() {
  ...
  return (
    <div className="linkedin-messages">
      <div className="linkedin-sidebar">
        <div className="sidebar-header"><h3>Messaging</h3></div>
        {loading ? (
          <p className="loading">Loading...</p>
        ) : users.length ? (
          users.map((u) => (
            <div
              key={u._id}
              className={`sidebar-user ${selectedUser?._id === u._id ? "active" : ""}`}
              onClick={() => handleSelectUser(u)}
            >
              <Avatar src={u.avatar || `https://api.dicebear.com/9.x/pixel-art/svg?seed=${u._id}`} alt="avatar" className="sidebar-avatar" />
              <div className="sidebar-user-info">
                <p className="sidebar-username">{u.firstName} {u.lastName}</p>
                {unreadByUser[u._id] > 0 && (
                  <span className="sidebar-unread-badge">{unreadByUser[u._id] > 9 ? "9+" : unreadByUser[u._id]}</span>
                )}
              </div>
            </div>
          ))
      ...
      <div className="linkedin-chat">
        {selectedUser ? (
          <>
            <div className="chat-header">
              <div className="chat-user-info">
                <Avatar src={selectedUser.avatar || `https://api.dicebear.com/9.x/pixel-art/svg?seed=${selectedUser._id}`} alt="avatar" className="chat-header-avatar" />
            ...
            <form onSubmit={handleSendMessage} className="chat-footer">
              <div className="chat-input-wrapper">
                <textarea
                  ...
                  className="chat-textarea"
                />
                <button type="submit" className={`chat-send-btn ${newMessage.trim() ? "active" : ""}`} disabled={!newMessage.trim()}>
                  <i className="fas fa-paper-plane"></i>
                </button>
              </div>
            </form>
          ...
          <div className="empty-chat"><p>Select a user to start chatting.</p></div>
        )}
      </div>
    </div>
  );
