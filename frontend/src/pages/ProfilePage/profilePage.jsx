...
import { getToken } from "../../services/authService";
import { getSafeAvatarUrl } from "../../components/Avatar/Avatar";
import {
  sendFriendRequest,
  getFriendRequests,
  acceptFriendRequest,
      ...
      try {
        ...
        setConnections(
          connList.map((c) => ({
            ...
            firstName: c.firstName || "",
            lastName: c.lastName || "",
            profileImage: getSafeAvatarUrl(c.avatar || c.profileImage, c._id),
          }))
        );
  ...
  return (
    <div className="profile-page">
      <div className="profile-card main">
        ...
        {profile.occupation && <p><Briefcase size={16} /> {profile.occupation}</p>}
        {profile.education && <p><GraduationCap size={16} /> {profile.education}</p>}
        {profile.bio && <p className="profile-bio"><MessageCircle size={16} /> {profile.bio}</p>}
      ...
      {authUser?._id === profile._id && incomingRequests.length > 0 && (
        <div className="profile-card">
          <h3>Connection Requests</h3>
          <ul className="requests-list">
            {incomingRequests.map((r) => (
              <li key={r._id} className="friend-request-item">
                ...
                <div className="request-buttons">
                  <button onClick={() => handleAcceptRequest(r._id)} className="accept-btn">Accept</button>
                  <button onClick={() => handleDeclineRequest(r._id)} className="reject-btn">Reject</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      <div className="profile-card">
        ...
        {connections.length === 0 ? <p>No connections yet.</p> : (
          <ul className="connections-list">
            {connections.map((c) => (
              <li key={c._id} className="connection-item" onClick={() => navigate(`/profile/${c._id}`)}>
                <div className="connection-left">
                  <Avatar src={c.profileImage} alt={`${c.firstName} ${c.lastName}`} className="connection-avatar" />
                  <span className="connection-name">{c.firstName} {c.lastName}</span>
                </div>
                <button onClick={(e) => { e.stopPropagation(); handleMessageUser(c._id); }} className="message-btn">Message</button>
              </li>
            ))}
          </ul>
        )}
      ...
      {authUser?._id === profile._id && (
        <div className="profile-card">
          <PotentialConnections userId={authUser._id} currentUserId={authUser._id} />
        </div>
      )}

      <div className="profile-card">
        <h3><HeartHandshake size={18} /> Help Sections</h3>
        <HelpSections canHelpWith={profile.canHelpWith} needHelpWith={profile.needHelpWith} />
      </div>

      {authUser?._id === profile._id && (
        <div className="profile-actions">
          <Link to="/edit-profile"><button className="btn btn-edit">✏️ Edit Profile</button></Link>
        </div>
      )}
    </div>
  );
